...
def readBatchData(dataFolder, batchLabel, loadAll=False, saveAll=True, vars=None, maxCombs=None, listCombs=None):
    ...
    if b['method'] == 'grid':
        ...
        return params, data
#--------------------------------------------------------------------
#Function to collect epsp amplitude vs parameters
#--------------------------------------------------------------------
Loclist=[]
Seclist=[]
epsplist=[]
...
def epsp(params, data):
    ...
    for key, d in data.iteritems():
        ...
        epsp = max(vsoma[300:500]) - vsoma[299]
        epsplist.append(epsp)
        dumpepsp.append({str(d['paramValues']): epsp})
        Loclist.append(d['paramValues'][1])
        Seclist.append(d['paramValues'][0])
#       - Do Weights and Locs later
#--------------------------------------------------------------------
...
#--------------------------------------------------------------------
def readPlot():
    dataFolder = '/Users/project/Neuro/M1NetworkModel/sim/data/Data_4'
    batchLabel = 'Data_4'
    
    params, data = readBatchData(dataFolder, batchLabel, loadAll=0, saveAll=1, vars=None, maxCombs=None)
    epsp(params, data)
    
    filename = '%s/%s_epsp.json' % (dataFolder, batchLabel)
    with open(filename, 'w') as fileObj:
        json.dump(dumpepsp, fileObj)
...
dataFolder = '/Users/project/Neuro/M1NetworkModel/sim/data'
batchLabel = 'Data_4'
...
plt.savefig('%s/%s_locepsp.png' % (dataFolder, batchLabel))
plt.show

"""plt.figure()
plt.plot(Seclist, epsplist, 'o', markersize = 3)
plt.xlabel("Sections")
plt.ylabel("Somatic EPSP amplitude")
plt.savefig('%s/%s_secepsp.png' % (dataFolder, batchLabel))
plt.show
"""
